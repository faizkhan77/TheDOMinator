...
export const getRating = async (question, answer) => {
  try {
    ...
    const suggestion = suggestionMatch ? suggestionMatch[1] : "No suggestion provided.";
    ...
    return { rating: "N/A", suggestion: "Could not process rating.", status: "Unknown" };
  }
};
...
export const getFinalFeedback = async (skill, ratings) => {
  try {
    const validRatings = Object.values(ratings).map(r => parseInt(r.rating)).filter(r => !isNaN(r));
    ...
    const result = await model.generateContent(prompt);
    return result.response.candidates[0]?.content?.parts[0]?.text.trim() || "Error fetching feedback.";
  } catch (error) {
